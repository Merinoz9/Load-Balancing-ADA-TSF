...
public class MST extends RigidStgy {
		...
		public AllocationEntry schedule(Job job, JobControlBlock jcb) {
			...
			List<UUID> admissibleSet = jobFilteringStgy.getKnowMachines();
			...
			for( UUID machine : admissibleSet ) {
				...
				double c = ((JRTEstimate)jreData.get(machine)).earliestStartTime.get(job).timestamp();
				if( c < minCompletion ) {
					minCompletion = c;
